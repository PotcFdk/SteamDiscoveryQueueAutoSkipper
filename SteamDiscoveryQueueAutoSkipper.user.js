...
const HELPERS = {
	...
	isLoggedIn:  () => !hasLoginLink() &&  hasAvatarClassElements(),
	isLoggedOut: () =>  hasLoginLink() && !hasAvatarClassElements(),
...
const HANDLERS = [
	...
	['discoveryQueueDetectAndPrompt', () => {
		if (isLoggedIn() && (Date.now() - (localStorage.SteamDiscoveryQueueAutoSkipper_lastchecked || 0) > HOUR)) {
	...
	['itemRewards', () => {
		if (isLoggedIn() && (Date.now() - (localStorage.SteamDiscoveryQueueAutoSkipper_freesticker_next_claim_time || 0) > 0)) {
...
for (let idx = 0; idx < HANDLERS.length; ++idx) {
	...
	console.log (`Handler #${idx} / ${handler[0]} -> ${result}`);
